...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log("1. LOGIN SUCCESS. Data received:", data);

            // Set cookies
            document.cookie = `userEmail=${data.email}; path=/; max-age=604800; SameSite=Lax`;
            document.cookie = `userName=${data.name}; path=/; max-age=604800; SameSite=Lax`;
                
            console.log("2. COOKIES SET. Current document.cookie:", document.cookie); // <--- CHECK IF userEmail IS HERE
            ...
            if (userType === 'student' && data.type) {
                ...
                // document.cookie = `userEmail=${data.email}; path=/; max-age=604800; SameSite=Lax`;
                // document.cookie = `userName=${data.name}; path=/; max-age=604800; SameSite=Lax`;
                // document.cookie = `userType=${userType}; path=/; max-age=604800; SameSite=Lax`;
                if (data.teamId) {
                    document.cookie = `teamId=${data.teamId}; path=/; max-age=604800; SameSite=Lax`;
                }
                        }
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
            {/* ADDED 'relative' */}
            <div className="max-w-md w-full bg-white border-2 border-gray-300 rounded-xl p-8 shadow-sm relative">
                
                {/* --- ADDED THIS BLOCK --- */}
                <Link
                    href={userType === 'student' ? '/studentfaq' : '/facultyfaq'}
                    className="absolute top-4 right-4 w-8 h-8 bg-gray-100 border-2 border-gray-300 rounded-full flex items-center justify-center text-gray-600 hover:text-red-800 hover:border-red-800 transition-all"
                    title="View FAQ"
                >
