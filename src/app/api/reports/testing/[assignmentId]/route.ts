...
// Helper function to fetch an image from S3
...
async function generateTestReport(assignmentId: string): Promise<{pdfBuffer: Buffer, projectTitle: string}> {
    ...
    // --- FIX 2: Manually register REAL .ttf fonts ---
    try {
        const fontDir = path.join(process.cwd(), 'fonts');
        
        // We are loading Roboto, but *registering it* under the name 'Helvetica'
        // This acts as an alias, so all your .font('Helvetica') calls will work.
        ...
        throw new Error("Failed to initialize PDF fonts. Check /fonts directory for Roboto-Regular.ttf and Roboto-Bold.ttf.");
    }
    // --- END: FONT FIX ---

    // --- FIX 3: Manually add the first page and set default font ---
    doc.addPage();
    doc.font('Helvetica'); // Set the default font for the document

    // --- Promise to capture stream ---
    ...
    // --- PDF Content (This part should now work) ---
    ...
    doc.text(`GitHub: `, { continued: true }).fillColor('blue').text(data.project.githubLink || "N/A", { link: data.project.githubLink });
    doc.fillColor('#000').text(`Live URL: `, { continued: true }).fillColor('blue').text(data.project.deployedLink || "N/A", { link: data.project.deployedLink });
    doc.fillColor('#000').text(`SRS: `, { continued: true }).fillColor('blue').text(data.project.srsLink || "N/A", { link: data.project.srsLink });
    doc.moveDown(1.5);

    // --- Section 2: Submitting Team ---
    doc.fillColor('#000').fontSize(16).font('Helvetica-Bold')
       .text('2. Submitting Team');
    doc.moveDown(0.5);
    doc.fontSize(12).font('Helvetica-Bold').text(data.originalTeam.teamName);
    doc.font('Helvetica').list(data.originalTeam.members?.map(m => 
        `${m.name} (${m.email}) ${m.isLeader ? '[LEADER]' : ''}`
    ) ?? [], { bulletRadius: 2 });
    doc.moveDown(1.5);

    // --- Section 3: Testing Team ---
    doc.fillColor('#000').fontSize(16).font('Helvetica-Bold')
       .text('3. Testing Team');
    doc.moveDown(0.5);
    doc.fontSize(12).font('Helvetica-Bold').text(data.testingTeam.teamName);
    doc.font('Helvetica').list(data.testingTeam.members?.map(m => 
        `${m.name} (${m.email}) ${m.isLeader ? '[LEADER]' : ''}`
    ) ?? [], { bulletRadius: 2 });
    doc.moveDown(1.5);
    ...
    doc.fillColor('#000').fontSize(16).font('Helvetica-Bold')
       .text('4. Test Case Results');
    doc.moveDown(0.5);

    for (const [index, tc] of (data.testCases ?? []).entries()) {
        if (index > 0) doc.addPage().font('Helvetica'); // Ensure font is reset on new pages
        
        doc.fontSize(14).font('Helvetica-Bold')
           .text(`Test Case ${index + 1}: ${tc.testcaseName}`);
        doc.moveDown(0.5);

        const statusColor = tc.testStatus === 'pass' ? 'green' : (tc.testStatus === 'fail' ? 'red' : 'gray');
